...
async function generateExportsMap(
  ...
  // Start with existing exports if they exist
  const exportsMap: ExportsMap = packageJson.exports ? { ...packageJson.exports } : {};
...
async function updatePackageJson(packageJsonPath: string, exportsMap: ExportsMap): Promise<boolean> {
